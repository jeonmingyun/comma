...
$(function() {
	...
	$(document).on(
			...
			function() {
				...
				var path = encodeURIComponent(inq_uploadpath + "/" + inq_uuid
						+ "_" + inq_filename);
				$.ajax({
					url : '/deleteFile',
					data : {
									...
									success : function(result) {
										...
										$("#uploadFile")
												.append(
														"<a href='#'>"
																+ result[0].fileName
																+ "</a><button id='deleteFile'>삭제하기</button>");
	...
	function uploadSuccess(uploadResultArr) {
		...
		$(uploadResultArr).each(
				function(i, obj) {
					str += "<input type='hidden' name='inq_filename' value='"
							+ obj.fileName + "'>";
					str += "<input type='hidden' name='inq_uuid' value='"
							+ obj.uuid + "'>";
					str += "<input type='hidden' name='inq_uploadpath' value='"
							+ obj.uploadPath + "'>";
				});
